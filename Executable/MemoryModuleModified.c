...
static BOOL
CopySections(const unsigned char *data, size_t size, PIMAGE_NT_HEADERS32 old_headers, PMEMORYMODULE module)
{
    int i, section_size;
...
FinalizeSections(PMEMORYMODULE module)
{
    ...
    // 64bits again.
        DWORD imageOffset = ((DWORD) module->headers->OptionalHeader.ImageBase & 0xffffffff00000000);
#else
    ...
    for (i=1; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
        LPVOID sectionAddress = (LPVOID)((DWORD)section->Misc.PhysicalAddress | imageOffset);
        ...
        if (sectionData.alignedAddress == alignedAddress || (DWORD) sectionData.address + sectionData.size > (DWORD) alignedAddress) {
            ...
            sectionData.size = (((DWORD)sectionAddress) + ((DWORD) sectionSize)) - (DWORD) sectionData.address;
            continue;
        }
...
BuildImportTable(PMEMORYMODULE module)
{
    ...
    for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
        ...
        for (; *thunkRef; thunkRef++, funcRef++) {
            if (IMAGE_SNAP_BY_ORDINAL32(*thunkRef)) {
    ...
    void *userdata)
{
    ...
    if (!CheckSize(size, dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS32))) {
        return NULL;
    }
    old_header = (PIMAGE_NT_HEADERS32)&((const unsigned char *)(data))[dos_header->e_lfanew];
   ...
   // TLS callbacks are executed BEFORE the main loading
    ...
    if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
        if (result->isDLL) {
            ...
            BOOL successfull = 1;// (*DllEntry)((HINSTANCE)code, DLL_PROCESS_ATTACH, 0);
            if (!successfull) {
